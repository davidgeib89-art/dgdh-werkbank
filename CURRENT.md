...
focus: `dgdh-skill-contract-and-verify-v1`; der Primitive-Gate ist jetzt repo-wahr erfuellt: nach Same-Session-Resume-Beweis und nativer CEO-Issue-Handoff-Primitive ist der naechste echte Hebel nicht mehr Shell-/`curl.exe`-Glue, sondern governte durable Capability fuer bereits bewiesene wiederkehrende Standardpfade
...
next:
  ...
  2) als erste Seed-Kandidaten nur bereits bewiesene Standardpfade nehmen, zuerst den CEO-native-issue-handoff-Pfad, spaeter erst weitere Resume-/Operator-Truth-Prozeduren falls der Scope klein bleibt
  3) erst nach diesem Sprint wieder urteilen, ob lokale Skill-Registry, Replay-backed Promotion oder `child-created`-Resume-Edge-Cases wirklich `Core` statt `Later` sind
...
notes:
  ...
  - `dgdh-skill-contract-and-verify-v1` ist jetzt der naechste wahrscheinliche `Core`-Sprint, aber nur eng geschnitten als `Contract + Verify + Promotion-State`; Registry, Router, Replay-Promotion und Evolution-Lane bleiben bewusst `Later`
