...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Paperclip skills into Cursor skills home", async () => {
    ...
    expect(await fs.readlink(injectedA)).toBe(path.join(skillsDir, "paperclip"));
    expect(await fs.readlink(injectedB)).toBe(path.join(skillsDir, "paperclip-create-agent"));
