...
- **Aktiver Agenten-/Paperclip-Workspace:** `C:\Users\holyd\DGDH\worktrees\astro-keystatic-template-geib`
...
- Fuer Revenue Lane ist die aktuelle Reihenfolge: Image Preprocessing -> Content Extraction/Draft -> Schema Fill -> kontrollierter Template-Apply -> Review
- Diese Foundation ist jetzt bis zum sicheren Template-Build bewiesen; der naechste Architektur-Schritt ist `Packet -> Lane Routing`, damit die Engine pro Packet bewusst zwischen `deterministic_tool`, `free_api`, `premium_model` und spaeter `local_model` waehlt
...
- **Dynamische Toolsets per Rolle + Packet-Typ** — Stage-1-Classifier steuert nicht nur Bucket/Modell sondern auch das Toolset des Workers: `bounded-implementation` → `[write_file, git_commit, create_pr]`, `heavy-architecture` → `[web_search, delegate, review]`. Direkte Inspiration: Hermes Agent `toolset_distributions.py`. Nach erstem E2E-Lauf. Details: `doc/research/2026-03-23-hermes-agent-research.md`
- **Skills als User-Messages (Cache-Optimierung)** — Role-Templates als User-Message statt System-Prompt injizieren. Spart Token-Kosten bei langen Runs erheblich. Erst wenn Token-Kosten messbar werden. Inspiration: Hermes Agent Skill-Injection Pattern. Details: `doc/research/2026-03-23-hermes-agent-research.md`
- **Telegram-Delivery fuer CEO-Summary** — David bekommt nach erstem vollautomatischen Merge eine Telegram-Nachricht. Das ist der Moment wo DGDH sich wie eine echte Firma anfuehlt. Nach erstem echten E2E-Lauf. Inspiration: Hermes Agent Gateway. Details: `doc/research/2026-03-23-hermes-agent-research.md`
- **Firm Memory Agent** — Background-Agent der abgeschlossene Issues + Verdicts liest, Muster via Flash-Lite extrahiert, und Learnings als `additionalContext` in CEO/Worker/Reviewer-Prompts injiziert. Drei Schichten: episodisch (existiert), semantisch (das ist Firm Memory), prozedural (Skill-Creation Engine). Learnings bekommen Verfallsdatum + Konfidenz-Gewichtung. Trigger: erst nach 10+ echten Runs. Inspiration: claude-subconscious (Letta). Details: `doc/backlog/firm-memory-agent.md`
