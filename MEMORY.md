...
- Gemini-Routing ist jetzt kleiner und ehrlicher: der Flash-Lite-Vorrouter ist standardmaessig auf billige `worker`/`reviewer`-`free_api`-Pfade begrenzt, CEO-Runs folgen ohne expliziten Packet-Pin wieder primaer der Task-/Bucket-Wahrheit, und wenn ein Lauf wirklich eine Lane pinnt wird auch `model = auto` auf die explizite Lane mutiert statt als verstecktes `auto-gemini-3` stehenzubleiben.
