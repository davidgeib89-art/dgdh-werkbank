...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    ...
    "- list child issues: `pnpm paperclipai issue list --company-id $env:PAPERCLIP_COMPANY_ID --parent-id $env:PAPERCLIP_TASK_ID --json`",
    "- list agents: `pnpm paperclipai agent list --company-id $env:PAPERCLIP_COMPANY_ID --json`",
    "- create child issue: `pnpm paperclipai issue create --company-id $env:PAPERCLIP_COMPANY_ID --project-id $env:PAPERCLIP_PROJECT_ID --parent-id $env:PAPERCLIP_TASK_ID --title \"...\" --description \"...\" --status todo --json`",
    "- assign child issue: `pnpm paperclipai issue assign CHILD_ISSUE_ID --agent-id WORKER_AGENT_ID --json`",
    "Run them from the repo root shown by `PAPERCLIP_CLI_CWD` so the workspace package manager context is correct.",
    ...
    `  command_execution({ command: "pnpm paperclipai issue list --company-id $env:PAPERCLIP_COMPANY_ID --parent-id $env:PAPERCLIP_TASK_ID --json", cwd: "$env:PAPERCLIP_CLI_CWD" })`,
    `  command_execution({ command: "pnpm paperclipai agent list --company-id $env:PAPERCLIP_COMPANY_ID --json", cwd: "$env:PAPERCLIP_CLI_CWD" })`,
    `  command_execution({ command: "pnpm paperclipai issue create --company-id $env:PAPERCLIP_COMPANY_ID --project-id $env:PAPERCLIP_PROJECT_ID --parent-id $env:PAPERCLIP_TASK_ID --title \"Example packet\" --description \"Titel: Example packet\npacketType: free_api\nexecutionIntent: implement\nreviewPolicy: required\nneedsReview: true\nstatus: todo\nZiel: ...\nScope: ...\ntargetFile: n/a\ntargetFolder: doc\nartifactKind: doc_update\ndoneWhen: ...\nAnnahmen:\n[NEEDS INPUT]: none\" --status todo --json", cwd: "$env:PAPERCLIP_CLI_CWD" })`,
    `  command_execution({ command: "$packet = @'\nTitel: Example packet\npacketType: free_api\nexecutionIntent: implement\nreviewPolicy: required\nneedsReview: true\nstatus: todo\nZiel: ...\nScope: ...\ntargetFile: n/a\ntargetFolder: doc\nartifactKind: doc_update\ndoneWhen: ...\nAnnahmen:\n[NEEDS INPUT]: none\n'@; pnpm paperclipai issue create --company-id $env:PAPERCLIP_COMPANY_ID --project-id $env:PAPERCLIP_PROJECT_ID --parent-id $env:PAPERCLIP_TASK_ID --title \"Example packet\" --description $packet --status todo --json", cwd: "$env:PAPERCLIP_CLI_CWD" })`,
    `  command_execution({ command: "pnpm paperclipai issue assign CHILD_ISSUE_ID --agent-id WORKER_AGENT_ID --json", cwd: "$env:PAPERCLIP_CLI_CWD" })`,
    "",
    "",
  ].join("\n");
}
